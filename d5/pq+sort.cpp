...
void reorderQueue(std::vector<int> &pages, const std::vector<std::pair<int, int>> &rules)
{
    std::stable_sort(pages.begin(), pages.end(), [&](int a, int b) {
        ...
        return false; // No specific order defined, keep the current order
    });
	std::cout << "after reordering: " << isValidQueue(pages,rules) << std::endl; 
}
